...
class Kinetics700(Dataset):
    def __init__(
        ...
        classes = [os.path.basename(i)
                   for i in glob.glob(os.path.join(ft_dir, "*"))]
    ...
    def __getitem__(self, index):
        ...
        if self.spatial_transform is not None:
            ...
            clip = torch.stack([self.spatial_transform(img)
                                for img in clip], 1)
...
def get_stats():
    ...
    maximum = 255.0
    mean = [110.63666788 / maximum, 103.16065604 /
            maximum, 96.29023126 / maximum]
